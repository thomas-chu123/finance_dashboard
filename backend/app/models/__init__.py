...
class PortfolioShareRequest(BaseModel):
    """建立投組分享的請求。"""
    share_type: str = "snapshot"  # "snapshot" 或 "public"
    expires_in_days: Optional[int] = 30  # None = 不過期
...
class PublicPortfolioShareResponse(BaseModel):
    """公開查看分享投組的響應（無認證）。"""
    share_key: str
    portfolio: dict  # 完整投組數據
    shared_by: Optional[str] = None  # 可匿名
    created_at: str
    view_count: int


class UserShareListResponse(BaseModel):
    """用戶分享列表項目。"""
    share_key: str
    portfolio_id: str
    portfolio_name: str
