...
async def get_shared_image(image_hash: str):
    """
    獲取無認證分享的PNG圖形
    - image_hash: 圖像哈希值，格式可以是 backtest_a1b2c3d4e5f6g7h8
    """
    try:
...
async def upload_result_image(
    ...
    result_type: str = Query(..., description="圖形類型: backtest, optimize, monte_carlo"),
    portfolio_id: Optional[str] = Query(None, description="投組ID"),
    authorization: str = Header(None),
):
    """
    上傳回測/優化/蒙地卡羅結果PNG圖形
    - file: PNG圖檔
    - result_type: 圖形類型（backtest/optimize/monte_carlo）
    - portfolio_id: 投組ID（用於記錄）
    """
    try:
