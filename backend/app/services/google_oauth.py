"""Google OAuth 用戶管理服務."""
import logging
from typing import Optional, Dict, Any
from app.database import get_supabase
import uuid

logger = logging.getLogger("google_oauth")


class GoogleOAuthUserService:
    """Google OAuth 用戶管理."""
    
    @staticmethod
    def get_or_create_user(google_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        根據 Google 數據獲取或創建用户.
        
        Args:
            google_data: Google ID token payload
            
        Returns:
            用戶資料字典
        """
        sb = get_supabase()
        email = google_data.get("email")
        google_id = google_data.get("sub")
        display_name = google_data.get("name", email.split("@")[0] if email else "User")
        picture_url = google_data.get("picture")
        
        if not email or not google_id:
            logger.error("缺少必要的 Google 用戶數據")
            raise ValueError("缺少 email 或 google_id")
        
        try:
            # 尋找現有用戶
            res = sb.table("profiles").select("*").eq("email", email).execute()
            if res.data:
                user = res.data[0]
                logger.info(f"找到現有用戶: {email}")
                
                # 如果沒有 google_id，則更新
                if not user.get("google_id"):
                    sb.table("profiles").update({
                        "google_id": google_id,
                        "oauth_provider": "google",
                        "picture_url": picture_url
                    }).eq("id", user["id"]).execute()
                    logger.info(f"更新用戶 google_id: {email}")
                
                return user
            
            # 創建新用戶
            new_user_id = str(uuid.uuid4())
            new_user_data = {
                "id": new_user_id,
                "email": email,
                "display_name": display_name,
                "google_id": google_id,
                "oauth_provider": "google",
                "picture_url": picture_url,
                "hashed_password": None  # OAuth 用戶無需密碼
            }
            
            insert_res = sb.table("profiles").insert(new_user_data).execute()
            if insert_res.data:
                logger.info(f"創建新 Google 用戶: {email} (ID: {new_user_id})")
                return insert_res.data[0]
            
            logger.error(f"創建用戶失敗: {email}")
            raise Exception("用戶創建失敗")
            
        except Exception as e:
            logger.error(f"用戶操作失敗: {str(e)}")
            raise
