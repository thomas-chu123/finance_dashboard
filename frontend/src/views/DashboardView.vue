<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-2">
      <div v-for="q in quotes" :key="q.symbol" @click="openQuoteUrl(q.symbol)" class="glass-card p-3.5 rounded-xl flex items-center justify-between cursor-pointer hover:border-emerald-500/50 transition-colors group">
        <div class="flex flex-col pr-3 overflow-hidden">
          <span class="text-xs font-bold text-[var(--text-primary)] group-hover:text-emerald-600 dark:group-hover:text-emerald-400 truncate">{{ q.name }}</span>
        ...
        <div class="flex flex-col items-end shrink-0">
          ...
          <span v-if="q.change !== null" :class="[
            ...
            q.change > 0 ? 'text-rose-600 dark:text-rose-400' : q.change < 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-500'
          ]">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
      ...
      <div class="xl:col-span-3 space-y-6">
        ...
        <div class="glass-card rounded-2xl overflow-hidden">
          <div class="p-6 border-b border-[var(--border-color)] flex items-center justify-between">
            ...
            <router-link to="/tracking" class="text-xs text-emerald-600 dark:text-emerald-400 font-bold hover:underline flex items-center gap-1">
              查看全部 <ChevronRight :size="14" />
            </router-link>
          </div>
          <div class="overflow-x-auto">
            <div v-if="trackingStore.loading" class="p-12 flex justify-center"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-500"></div></div>
            <div v-else-if="!trackingStore.items.length" class="p-12 text-center text-zinc-500">
              尚未追蹤任何指數 · <router-link to="/tracking" class="text-emerald-500 hover:underline">立即新增</router-link>
            </div>
            <div v-else class="min-w-[600px]">
              <div class="grid grid-cols-6 p-4 bg-zinc-50 dark:bg-zinc-900/30 text-[10px] uppercase font-bold tracking-widest text-zinc-500 border-b border-[var(--border-color)]">
              ...
              <div v-for="item in trackingStore.items.slice(0, 6)" :key="item.id" class="grid grid-cols-6 items-center p-4 border-b border-[var(--border-color)] hover:bg-zinc-100 dark:hover:bg-zinc-800/30 transition-colors">
                <div class="col-span-1 font-bold text-sm tracking-tight text-emerald-600 dark:text-emerald-400">{{ item.symbol }}</div>
                ...
                <div class="col-span-1 text-right font-mono text-sm text-[var(--text-primary)]">
                  <span v-if="item.trigger_price">
                    ...
                    <TrendingDown v-else :size="12" class="inline text-emerald-500 mb-0.5" />
                    {{ item.trigger_price }}
                  </span>
                  <span v-else class="text-zinc-500">—</span>
                </div>
                <div class="col-span-1 text-right">
                  <span :class="['px-2 py-0.5 text-[10px] font-bold rounded uppercase tracking-wider', item.is_active ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-rose-500/10 text-rose-600 dark:text-rose-400']">
        ...
        <div class="glass-card rounded-2xl overflow-hidden">
          ...
          <div class="overflow-x-auto">
            ...
            <div v-else class="min-w-[600px]">
              <div class="grid grid-cols-7 p-4 bg-zinc-50 dark:bg-zinc-900/30 text-[10px] uppercase font-bold tracking-widest text-zinc-500 border-b border-[var(--border-color)]">
              ...
              <div v-for="log in trackingStore.alertLogs.slice(0, 8)" :key="log.id" class="grid grid-cols-7 items-center p-4 border-b border-[var(--border-color)] hover:bg-zinc-100 dark:hover:bg-zinc-800/30 transition-colors">
                ...
                <div class="col-span-1 text-right">
                  <span :class="['px-2 py-0.5 text-[10px] font-bold rounded uppercase tracking-wider', log.status === 'sent' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-rose-500/10 text-rose-600 dark:text-rose-400']">
                    {{ log.status }}
                  </span>
                </div>
              </div>
      ...
      <div class="space-y-6">
        ...
        <div class="glass-card rounded-2xl p-6 bg-gradient-to-br from-emerald-500/5 to-transparent border-emerald-500/20">
          ...
          <div class="grid grid-cols-2 gap-4 mb-6">
            <div class="flex flex-col gap-1 p-4 bg-zinc-50 dark:bg-zinc-900/50 rounded-xl border border-[var(--border-color)]">
              ...
              <span class="text-[10px] text-emerald-500 font-bold">{{ activeCount }} 啟用中</span>
            </div>
            <div class="flex flex-col gap-1 p-4 bg-zinc-50 dark:bg-zinc-900/50 rounded-xl border border-[var(--border-color)]">
          ...
          <div class="space-y-3">
            <div class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg border border-[var(--border-color)]">
              <div class="flex items-center gap-2">
                <Mail :size="16" class="text-zinc-400" />
                <span class="text-sm font-bold text-[var(--text-primary)]">Email 通知</span>
              </div>
              <span :class="['text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded', auth.profile?.notify_email ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-zinc-200 dark:bg-zinc-800 text-zinc-500']">
            ...
            <div class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg border border-[var(--border-color)]">
              ...
              <span :class="['text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded', auth.profile?.notify_line ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-zinc-200 dark:bg-zinc-800 text-zinc-500']">
                {{ auth.profile?.notify_line ? '已啟用' : '停用' }}
              </span>
            </div>
          </div>
          
          <router-link to="/line" class="block w-full mt-6 py-3 border border-[var(--border-color)] rounded-xl text-xs font-bold text-center text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors">
            前往設定通知
          </router-link>
        </div>
    ...
    <div v-if="showQuoteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="showQuoteModal = false">
      <div class="bg-[var(--bg-main)] border border-[var(--border-color)] rounded-2xl shadow-2xl w-[860px] max-w-[96vw] max-h-[90vh] flex flex-col">
        ...
        <div class="flex-1 overflow-hidden grid grid-cols-1 md:grid-cols-3 gap-6 p-6 min-h-0">
          ...
          <div class="md:col-span-2 flex flex-col min-h-0">
            ...
            <div class="flex gap-2 mb-4 flex-wrap">
              <button
                ...
                :class="['px-4 py-1.5 rounded-full border text-sm font-bold transition-colors', activeTab === tab.key ? 'bg-emerald-500 border-emerald-500 text-white dark:text-black' : 'border-[var(--border-color)] text-zinc-500 hover:border-emerald-500 hover:text-emerald-500']"
            ...
            <div class="mb-3 relative">
              ...
              <input
                type="text" class="w-full bg-zinc-100 dark:bg-zinc-900 border border-[var(--border-color)] rounded-lg py-2 pl-9 pr-4 text-sm focus:outline-none focus:border-emerald-500/50 transition-colors text-[var(--text-primary)]" v-model="quoteSearch"
            ...
            <div v-else class="flex-1 overflow-y-auto border border-[var(--border-color)] rounded-xl custom-scrollbar bg-zinc-50 dark:bg-zinc-900/30">
              <div
                v-for="item in filteredCurrentTab" :key="item.symbol"
                class="flex justify-between items-center p-3 text-sm border-b border-[var(--border-color)] hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors"
                :class="{ 'opacity-50': isSelected(item.symbol) }"
              >
                <div class="flex items-baseline gap-2 overflow-hidden pr-2">
                  <span class="font-bold text-emerald-600 dark:text-emerald-400 shrink-0">{{ item.symbol }}</span>
                  <span class="text-xs text-zinc-500 truncate max-w-[200px]">{{ item.name }}</span>
                </div>
                <button
                  v-if="!isSelected(item.symbol)"
                  class="p-1 rounded text-emerald-600 hover:bg-emerald-500/10 font-bold shrink-0"
                  @click="addQuote(item)"
                ><Plus :size="16" /></button>
                <span v-else class="text-xs text-zinc-500 font-bold shrink-0 flex items-center gap-1"><Check :size="14" />已加入</span>
              </div>
          ...
          <!-- RIGHT: Selected list -->
          <div class="md:col-span-1 flex flex-col border border-emerald-500/30 rounded-xl overflow-hidden min-h-0">
            <div class="flex items-center justify-between p-3 border-b border-[var(--border-color)] bg-emerald-500/5 shrink-0">
              <span class="font-bold text-sm text-[var(--text-primary)]">已追蹤指數</span>
              <span class="bg-emerald-500 text-white dark:text-black text-[10px] font-bold px-2 py-0.5 rounded-full">{{ selectedQuotes.length }}</span>
            </div>
            <div class="flex-1 overflow-y-auto custom-scrollbar bg-zinc-50 dark:bg-zinc-900/30">
              <div
        ...
        <div class="p-4 px-6 border-t border-[var(--border-color)] flex items-center justify-end gap-3 bg-[var(--bg-header)] shrink-0">
          ...
          <button class="px-6 py-2 bg-emerald-500 hover:bg-emerald-600 text-white dark:text-black text-sm font-bold rounded-xl transition-all shadow-lg shadow-emerald-500/20 flex items-center gap-2" @click="saveQuotes" :disabled="savingQuotes">
            <div v-if="savingQuotes" class="animate-spin rounded-full h-4 w-4 border-b-2 border-white dark:border-black"></div>
            儲存追蹤清單
          </button>
        </div>
      </div>
